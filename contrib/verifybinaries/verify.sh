...
###   chuckramcore.org and chuckram.org and compares them.
...
WORKINGDIR="/tmp/chuckram_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://chuckramcore.org"
HOST2="https://chuckram.org"
BASEDIR="/bin/"
VERSIONPREFIX="chuckram-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'chuckram-',
...
if [ $? -ne 0 ]; then
   echo "chuckram.org failed to provide signature file, but chuckramcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "chuckram.org and chuckramcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Chuckram Core binary release signing key installed?"
   fi
