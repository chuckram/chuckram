...
const char * const CHUCKRAM_CONF_FILENAME = "chuckram.conf";
const char * const CHUCKRAM_PID_FILENAME = "chuckramd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "chuckramd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Chuckram
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Chuckram
    // Mac: ~/Library/Application Support/Chuckram
    // Unix: ~/.chuckram
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Chuckram";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Chuckram";
#else
    // Unix
    return pathRet / ".chuckram";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CHUCKRAM_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", CHUCKRAM_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Chuckram Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Chuckram Core") == std::string::npos) {
